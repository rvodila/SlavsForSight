...
#   These RF data might need reordering depending if the order is channel or electrode wise
...
channel_map_exp1_dict['RFX'] = np.full_like(channel_map_exp1_dict['instanceChannels_1024'], np.nan, dtype=float)
channel_map_exp1_dict['RFY'] = np.full_like(channel_map_exp1_dict['instanceChannels_1024'], np.nan, dtype=float)
channel_map_exp1_dict['STDX'] = np.full_like(channel_map_exp1_dict['instanceChannels_1024'], np.nan, dtype=float)
channel_map_exp1_dict['STDY'] = np.full_like(channel_map_exp1_dict['instanceChannels_1024'], np.nan, dtype=float)
channel_map_exp1_dict['R2'] = np.full_like(channel_map_exp1_dict['instanceChannels_1024'], np.nan, dtype=float)

#depending on where the data comes from, we reorder array channel wise (using jumpindex) or channel wise (instanceChannels_1024)
if RF_TYPE == 'MATT' or RF_TYPE == 'THINGS':
  KEY_REORDER = 'jumpIndex'
else:
  KEY_REORDER = 'instanceChannels_1024'
...
for idx, channel in enumerate(channel_map_exp1_dict[KEY_REORDER]):
    
    # # finding where in the mapping dict is every channel
    # whereChann = np.where(channel_map_exp1_dict['instanceChannels_1024'] == channel)
    print(channel)
...
for area in set(channel_map_exp1_dict['Area']):
    ...
    plt.figure(dpi = 300)  # Create a new figure for each area
...
# Create a dictionary to track labels that have been added to avoid duplicate legend entries
label_dict = {}
...
for area in set(channel_map_exp1_dict['Area']):
    ...
    rfy = channel_map_exp1_dict['RFY'][indices]
    colors = channel_map_exp1_dict['arrayColor'][indices]
    array_numbers = channel_map_exp1_dict['arrayNumbers'][indices]  # Array numbers for each electrode

    # Plot each point and create a legend entry if not already added
    for x, y, color, array_num in zip(rfx, rfy, colors, array_numbers):
        label = f"Array {array_num}"
        ...
        if label not in label_dict:
            plt.scatter(x, y, color=color, label=label, alpha=0.9, s=60, marker=marker, edgecolors='black')
            label_dict[label] = color
        else:
            plt.scatter(x, y, color=color, alpha=0.9, s=60,  marker=marker, edgecolors='black')
    ...
    plt.ylabel('RF Y Location')
    plt.title(f'Receptive Field Locations in Area {area}')
    plt.legend(title="Arrays", title_fontsize='13', fontsize='10', loc='upper right')
    plt.grid(False)
    # plt.axis('equal')  # Set equal scaling by changing axis limits
...
# Preparing to plot
plt.figure(dpi=300)  # Create one figure for all areas

# Extract array numbers, RF locations, and colors
array_numbers = channel_map_exp1_dict['arrayNumbers']
rfx = channel_map_exp1_dict['RFX']
rfy = channel_map_exp1_dict['RFY']
colors = channel_map_exp1_dict['arrayColor']

# Unique arrays and their colors
unique_arrays = np.unique(array_numbers)
for array_num in unique_arrays:
    indices = np.where(array_numbers == array_num)  # Get indices for each unique array
    array_color = colors[indices][0]  # Assume all points in the same array share the same color
    label = f"Array {array_num}"

    # Plotting all points belonging to the current array
    plt.scatter(rfx[indices], rfy[indices], color=array_color, label=label, alpha=0.9, s=60, edgecolors='black')
...
#%% Plotting eccentricity vs size per area
...
for area in ['V1', 'V2', 'V4', 'IT']:
    ...
    if np.sum(valid_indices) > 0:  # Proceed only if there are valid data points
        ...
        r_squared = model.score(area_eccentricity.reshape(-1, 1), area_rf_size)
        ...
        plt.text(0.80, 0.95, f'R² = {r_squared:.2f}', ha='left', va='top', transform=plt.gca().transAxes)
...
for area in areas:
    ...
    model_rfx = make_pipeline(MinMaxScaler(), PolynomialFeatures(degree=2), RidgeCV(alphas=np.linspace(0.001, 100, 1000)))
    model_rfx.fit(X_train, Y_train_rfx)
    
    model_rfy = make_pipeline(MinMaxScaler(), PolynomialFeatures(degree=2), RidgeCV(alphas=np.linspace(0.001, 100, 1000)))
    model_rfy.fit(X_train, Y_train_rfy)
...
#%% Predicting in ecc and polar angle instead // linear fit
...
# Calculate RF eccentricity and polar angle for each channel
eccentricity = np.sqrt(np.array(channel_map_exp1_dict['RFX'])**2 + np.array(channel_map_exp1_dict['RFY'])**2)
polar_angle = np.arctan2(np.array(channel_map_exp1_dict['RFY']), np.array(channel_map_exp1_dict['RFX'])) * (180 / np.pi)
...
for area in areas:
    ...
    if np.sum(known_indices) > 0:  # Proceed only if there are known data points
        # Prepare training data in polar coordinates
        X_train = channel_map_exp1_dict['electrodeXYPositions'][known_indices]
        Y_train_eccentricity = eccentricity[known_indices]
        Y_train_polar_angle = polar_angle[known_indices]

        # Define and fit the models
        model_eccentricity = make_pipeline(MinMaxScaler(), RidgeCV(alphas=np.linspace(0.001, 100, 1000)))
        model_polar_angle = make_pipeline(MinMaxScaler(), RidgeCV(alphas=np.linspace(0.001, 100, 1000)))

        model_eccentricity.fit(X_train, Y_train_eccentricity)
        model_polar_angle.fit(X_train, Y_train_polar_angle)

        # Predict eccentricity and polar angle for unknown data
        X_unknown = channel_map_exp1_dict['electrodeXYPositions'][unknown_indices]
        predicted_eccentricity = model_eccentricity.predict(X_unknown)
        predicted_polar_angle = model_polar_angle.predict(X_unknown)

        # Convert predictions back to XY coordinates
        channel_map_exp1_dict['RFX_predicted'][unknown_indices] = predicted_eccentricity * np.cos(np.radians(predicted_polar_angle))
        channel_map_exp1_dict['RFY_predicted'][unknown_indices] = predicted_eccentricity * np.sin(np.radians(predicted_polar_angle))

        # Ensure known values are preserved in the predictions
        channel_map_exp1_dict['RFX_predicted'][known_indices] = channel_map_exp1_dict['RFX'][known_indices]
        channel_map_exp1_dict['RFY_predicted'][known_indices] = channel_map_exp1_dict['RFY'][known_indices]

# Plotting the predicted receptive fields
for area in areas:
    plt.figure(figsize=(8, 5))
    indices = np.array(channel_map_exp1_dict['Area']) == area
...
#%% Predicting in ecc and polar angle instead // Exponential fit
...
for area in areas:
    ...
    if np.sum(known_indices) > 0:  # Proceed only if there are known data points
        # Prepare training data in polar coordinates
        X_train = channel_map_exp1_dict['electrodeXYPositions'][known_indices]
        Y_train_eccentricity = np.log1p(eccentricity[known_indices])  # Apply logarithmic transformation
        Y_train_polar_angle = polar_angle[known_indices]

        # Define and fit the models
        model_eccentricity = make_pipeline(MinMaxScaler(), LinearRegression())
        model_polar_angle = make_pipeline(MinMaxScaler(), LinearRegression())
        ...
        # Predict eccentricity and polar angle for unknown data
        X_unknown = channel_map_exp1_dict['electrodeXYPositions'][unknown_indices]
        predicted_eccentricity = np.expm1(model_eccentricity.predict(X_unknown))  # Apply inverse transformation
        predicted_polar_angle = model_polar_angle.predict(X_unknown)
...
for area in areas:
    ...
    plt.grid(False)
    plt.show()
#%% Predicting ALL values with the exponential fit on ecc and angle
...
for area in areas:
    ...
    if np.sum(known_indices) > 0:  # Proceed only if there are known data points
        ...
        # Predict eccentricity and polar angle for all data in the current area
        X_all = channel_map_exp1_dict['electrodeXYPositions'][area_indices]
        predicted_eccentricity = np.expm1(model_eccentricity.predict(X_all))  # Apply inverse transformation
        predicted_polar_angle = model_polar_angle.predict(X_all)

        # Convert predictions back to XY coordinates
        channel_map_exp1_dict['RFX_predicted'][area_indices] = predicted_eccentricity * np.cos(np.radians(predicted_polar_angle))
        channel_map_exp1_dict['RFY_predicted'][area_indices] = predicted_eccentricity * np.sin(np.radians(predicted_polar_angle))
...
# Fit models for V1 and V2 together
v1_v2_indices = (np.array(channel_map_exp1_dict['Area']) == 'V1') | (np.array(channel_map_exp1_dict['Area']) == 'V2')
known_indices_v1_v2 = np.isfinite(channel_map_exp1_dict['RFX']) & v1_v2_indices

if np.sum(known_indices_v1_v2) > 0:
    X_train_v1_v2 = channel_map_exp1_dict['electrodeXYPositions'][known_indices_v1_v2]
    Y_train_eccentricity_v1_v2 = np.log1p(eccentricity[known_indices_v1_v2])
    Y_train_polar_angle_v1_v2 = polar_angle[known_indices_v1_v2]

    model_eccentricity_v1_v2 = make_pipeline(MinMaxScaler(), LinearRegression())
    model_polar_angle_v1_v2 = make_pipeline(MinMaxScaler(), LinearRegression())

    model_eccentricity_v1_v2.fit(X_train_v1_v2, Y_train_eccentricity_v1_v2)
    model_polar_angle_v1_v2.fit(X_train_v1_v2, Y_train_polar_angle_v1_v2)

    X_all_v1_v2 = channel_map_exp1_dict['electrodeXYPositions'][v1_v2_indices]
    predicted_eccentricity_v1_v2 = np.expm1(model_eccentricity_v1_v2.predict(X_all_v1_v2))
    predicted_polar_angle_v1_v2 = model_polar_angle_v1_v2.predict(X_all_v1_v2)

    channel_map_exp1_dict['RFX_predicted'][v1_v2_indices] = predicted_eccentricity_v1_v2 * np.cos(np.radians(predicted_polar_angle_v1_v2))
    channel_map_exp1_dict['RFY_predicted'][v1_v2_indices] = predicted_eccentricity_v1_v2 * np.sin(np.radians(predicted_polar_angle_v1_v2))

# Fit models for V4 and IT individually
for area in ['V4', 'IT']:
    area_indices = np.array(channel_map_exp1_dict['Area']) == area
    known_indices = np.isfinite(channel_map_exp1_dict['RFX']) & area_indices

    if np.sum(known_indices) > 0:
        X_train = channel_map_exp1_dict['electrodeXYPositions'][known_indices]
        Y_train_eccentricity = np.log1p(eccentricity[known_indices])
        Y_train_polar_angle = polar_angle[known_indices]
        ...
        X_all = channel_map_exp1_dict['electrodeXYPositions'][area_indices]
        predicted_eccentricity = np.expm1(model_eccentricity.predict(X_all))
        predicted_polar_angle = model_polar_angle.predict(X_all)
